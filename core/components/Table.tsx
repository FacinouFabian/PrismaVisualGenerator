...
const Table: React.FunctionComponent<Props> = (props): JSX.Element => {
  ...
  const saveTable = () => props.callback(name, fields);
      ...
      <div>
        ...
        <div className="w-20 relative">
          ...
          <button
            type="button"
            title="save table"
            className="flex absolute h-auto w-auto top-1/3 right-0 -mt-1 -mr-1 focus:outline-none"
            onClick={saveTable}
          >
            <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-green-400 opacity-75"></span>
            <span className="relative flex justify-center items-center rounded-full h-6 w-6 bg-green-500">
